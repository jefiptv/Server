...
namespace BlazorWithFirestore.Server.DataAccess
{
    public class EmployeeDataAccessLayer : IEmployee
    {
        ...
        {
            string filepath = "C:\\Banco de dados\\rhsindical.json";
            Environment.SetEnvironmentVariable("GOOGLE_APPLICATION_CREDENTIALS", filepath);
            projectId = "rhsindical-ec8dd";
        ...
        public async Task<List<Employee>> GetAllEmployees()
        {
            try
            {
                Query employeeQuery = fireStoreDb.Collection("Socios");
        ...
        public async void AddEmployee(Employee employee)
        {
            try
            {
                CollectionReference colRef = fireStoreDb.Collection("Socios");
        ...
        public async void UpdateEmployee(Employee employee)
        {
            try
            {
                DocumentReference empRef = fireStoreDb.Collection("Socios").Document(employee.EmployeeId);
        ...
        public async Task<Employee> GetEmployeeData(string id)
        {
            try
            {
                DocumentReference docRef = fireStoreDb.Collection("Socios").Document(id);
        ...
        public async void DeleteEmployee(string id)
        {
            try
            {
                DocumentReference empRef = fireStoreDb.Collection("Socios").Document(id);
